...
p <- p + facet_wrap( ~ N, ncol=2) + opts(legend.position = "none", axis.title.x = theme_text(size = 12), axis.title.y = theme_text(size = 12, angle = 90) ) + ylim(0,.75)
print(p)
dev.off()
...
p <- p + xlab("Iteration") + ylab(expression(paste("Log(",theta,")"))) + opts(legend.position = "none", axis.title.x = theme_text(size = 12), axis.title.y = theme_text(size = 12, angle = 90) )
