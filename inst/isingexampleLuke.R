...
#                                 storeall = FALSE)
mhparameters <- tuningparameters(nchains = 10, niterations = 1000000, 
                                 saveeverynth = 10000, computemean = TRUE) 
print(mhparameters)
...
densitybinningMH <- binning(position = getLogEnergy,
                          name = "minus log target density",
                          binrange = c(-5990, -5600),
                          ncuts = 9,
                          autobinning = FALSE, learningrate = function(x) 0)#
#
Rprof(tmp <- tempfile())
pawlresultsMH <- pawl(isingtarget, densitybinningMH, mhparameters, proposalinstance)
Rprof()
# display profiling results
print(summaryRprof(tmp))
unlink(tmp)

save(list = ls(all=TRUE), file = "IsingRuns.RData")
...
# Plots of Mean Exploration
X11()
image.plot(1:40, 1:40, 1 - meanchains, zlim=c(0,1), 
           col=gray((64:1)^2 / (64)^2), xlab=expression(X[1]), ylab=expression(X[2]))

X11();print(PlotLogTheta(pawlresultsMH))
meanchainsMH <- apply(pawlresultsMH$allchains, c(2, 3), mean)
meanchainsMH <- pawlresultsMH$meanchains
meanchainsMH <- matrix(apply(meanchainsMH, 2, mean), ncol = imgsize)
X11()
image.plot(1:40, 1:40, 1 - meanchainsMH, zlim=c(0,1), 
           col=gray((64:1)^2 / (64)^2), xlab=expression(X[1]), ylab=expression(X[2]))
...
melted1 = rbind(cbind(rep(1,1600), rep(3,1600), grid[,1], grid[,2], 1-pawlresultsMH$allchains[60,2,]),
cbind(rep(1,1600), rep(4,1600), grid[,1], grid[,2], 1-pawlresultsMH$allchains[70,2,]),
cbind(rep(1,1600), rep(5,1600), grid[,1], grid[,2], 1-pawlresultsMH$allchains[80,2,]),
cbind(rep(1,1600), rep(6,1600), grid[,1], grid[,2], 1-pawlresultsMH$allchains[90,2,]),
cbind(rep(1,1600), rep(7,1600), grid[,1], grid[,2], 1-pawlresultsMH$allchains[100,2,]) )
...
p + geom_tile() + facet_grid(Group ~ Sample) + xlab(expression(X[1])) + ylab(expression(X[2]))
dev.off()

           
           
           
#meltedMean = rbind(cbind(rep(1,1600), grid[,1], grid[,2], rowMeans(sapply(pwl_NoWL$allparticles, colMeans)[,2001:5000])), cbind(rep(2,1600), grid[,1], grid[,2], rowMeans(sapply(pwl_WL$allparticles, colMeans)[,2001:5000])))
meltedMean = rbind(cbind(rep(1,1600), grid[,1], grid[,2], apply(pawlresultsMH$allchains[60:100,,], 3, mean)), cbind(rep(2,1600), grid[,1], grid[,2], apply(pawlresults$allchains[60:100,,], 3, mean)))
...
p + geom_tile() + facet_grid(. ~ Group) + xlab(expression(X[1])) + ylab(expression(X[2])) + scale_fill_gradient(low = "white", high = "black")
dev.off()
